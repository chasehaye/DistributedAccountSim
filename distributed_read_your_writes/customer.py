    ...
    def executeEvents(self):
        ...
        for event in self.events:
            ...
            if event['interface'] == "query":
                ...
                result = {"interface": "query", "branch": branch_id, "balance": res.balance}
            elif event['interface'] == "deposit":
                # Include event_id for write tracking
                res = stub.Deposit(
                    banks_pb2.DepositRequest(customer_id=self.id, amount=event['money'], event_id=event_id)
                )
                # Append the write_id only if the operation was successful
                self.writeset.append(res.write_id)
                result = {"interface": "deposit", "branch": branch_id, "result": res.result}
            elif event['interface'] == "withdraw":
                # Include event_id for write tracking
                res = stub.Withdraw(
                    banks_pb2.WithdrawRequest(customer_id=self.id, amount=event['money'], event_id=event_id)
                )
                # Append the write_id only if the operation was successful
                self.writeset.append(res.write_id)
                result = {"interface": "withdraw", "branch": branch_id, "result": res.result}

            responses.append({"id": self.id, "recv": [result]})
